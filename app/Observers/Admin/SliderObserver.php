...
class SliderObserver
{
    ...
    public function created(Slider $slider)
    {
        ...
        if (request()->file('mobile_image')) {
            if (request()->get('old_mobile_image')) {
                $slider->deleteItemImage(request()->get('old_mobile_image'));
            }
            $mobileImageName = ImageUploader::upload(request()->file('mobile_image'), $slider::$imageSmallSizes);
