...
class Orderr extends AbstractModel
{
    ...
    public static function makeOrder($inputs)
    {
        ...
        $order->items()->attach($toAttachData);
    ...
    {
        return $this->belongsToMany(Item::class, 'ordered_items', 'order_id', 'item_id')
            ->withPivot('count', 'price', 'real_price', 'name', 'code', 'sum');
    }
    ...
    {
        return self::query()->where('status', $status)->delete();
    }

    public static function getCount($status)
    {
        return self::query()->where('status', $status)->count();
    }
    ...
    {
        $query = self::query()->where('user_id', User::auth()->id);
        if ($status !== null) $query = $query->where('status', $status);
        if ($only_count) return $query->count();
    ...
    {
        return self::query()->where(['id' => $id, 'user_id' => authUser()->id])->with('products')->first();
    }
