...
class ServicesControllerAdmin extends AdminBaseController
{
    public function main()
    {
        $data['title'] = t('Admin Sidebar.Services');
    ...
    public function add_put(Request $request)
    {
        ...
        if(Service::action(null, $validator['inputs'])) {
            ...
            return redirect()->route('admin.services.main');
        }
        else {
    ...
    public function delete(Request $request)
    {
        ...
        if ($id && is_id($id)) {
            ...
            if ($item && Service::deleteItem($item)) $result['success'] = true;
        }
    ...
    public function deleteImage(Request $request)
    {
        ...
        if ($id && is_id($id)) {
            $item = Service::where('id', $id)->first();
            if ($item && Service::deleteImage($item)) $result['success'] = true;
        }
        return response()->json($result);
    }
    ...
    private function validator($request, $ignore = false, $page = null)
    {
        ...
        $rules = [
            ...
            'image' => ($ignore ? 'nullable' : '').'|image|mimes:jpeg,png',
            'image_alt' => 'nullable|string|max:255',
            'image_title' => 'nullable|string|max:255',
            'icon' => ($ignore ? 'nullable' : '').'|image|mimes:jpeg,png',
