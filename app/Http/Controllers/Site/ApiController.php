...
class ApiController extends BaseController
{
        ...
        if (count($sponsorIds)) {
            foreach ($sponsorIds as $id) {
                $infoArray = [
                    ...
                    'Donor acoount number' => $id,
                    'Donor Email' => $this->getSponsorEmail($id),
     ...
     * @return mixed
     */
    private function getSponsorEmail(int $id)
    {
        $item = User::where('id', $id)->select('email')->first();
    ...
    private function getSponsorDonationDates(int $id)
    {
        $sponsorDonationDates = Donation::withTrashed()
            ->where(['sponsor_id' => $id, 'status' => 1])
            ->orderBy('created_at', 'asc')
            ->select(DB::raw('DATE_FORMAT(created_at, "%d/%m/%Y %H:%i:%s") as formatted_dob'))
            ->pluck('formatted_dob')
            ->toArray();
     ...
     * @return array
     */
    private function getSponsorDonationFundraisersTitles(int $id)
    {
        $sponsorDonationFundraisersTitlesArray = [];
        $sponsorDonationFundraisersTitles = Donation::withTrashed()
            ->where(['sponsor_id' => $id, 'status' => 1])->whereHas('fundraiser')->with('fundraiser')
            ->orderBy('created_at', 'asc')
            ->select('fundraiser_id')
            ->get();
        if ($sponsorDonationFundraisersTitles) {
            foreach ($sponsorDonationFundraisersTitles as $row => $sponsorDonationFundraisersTitle) {
                if ($sponsorDonationFundraisersTitle->fundraiser) {
                    array_push($sponsorDonationFundraisersTitlesArray, $sponsorDonationFundraisersTitle->fundraiser->title ?? null);
                }
            }
        }
        return $sponsorDonationFundraisersTitlesArray;
    }
     ...
     * @return array
     */
    private function getSponsorDonationTypes(int $id)
    {
        $sponsorDonationTypesArray = [];
        $sponsorDonationTypes = Donation::withTrashed()
            ->where(['sponsor_id' => $id, 'status' => 1])
            ->orderBy('created_at', 'asc')
            ->select('fundraiser_id', 'gift_id')
            ->get()
            ->toArray();
        foreach ($sponsorDonationTypes as $row => $sponsorDonationType) {
            if (isset($sponsorDonationType['gift_id'])) {
                array_push($sponsorDonationTypesArray, __('app.Donation.Gift'));
            } elseif (isset($sponsorDonationType['fundraiser_id'])) {
                array_push($sponsorDonationTypesArray, __('app.Donation.Fundraiser'));
            } else {
                array_push($sponsorDonationTypesArray, __('app.Donation.Other'));
            }
        }

        return $sponsorDonationTypesArray;
    }
