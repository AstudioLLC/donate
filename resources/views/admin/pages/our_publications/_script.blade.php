...
@push('js')
    ...
    <script>
        ...
        const showUrl = '{{ route('admin.our_publications.show', ['id' => ':slug']) }}';

        const galleryTooltip = '{!! __('app.Gallery') !!}';
        const galleryUrl = '{{ route('admin.gallery.index', ['gallery' => 'our_publications', 'key' => ':slug']) }}';

        const editTooltip = '{!! __('app.Edit') !!}';
        const editUrl = '{{ route('admin.our_publications.edit', ['id' => ':slug']) }}';

        const fileTooltip = '{!! __('app.Files') !!}';
        const fileUrl = '{{ route('admin.file.index', ['file' => 'our_publications', 'key' => ':slug']) }}';
        ...
        $('.dataTable').dataTable({
            ...
            //order: [],
            ajax: {
                url: '{{ route('admin.our_publications.listPortion') }}',
                type: 'POST',
                /*data: function (e) {
        ...
        $(document).on('click', '.active-changer span', function() {
            ...
            let url = "{!! route('admin.our_publications.active', ['id' => ':slug']) !!}";
        ...
        $('#itemDeleteForm').on('submit', function () {
            let url = "{!! route('admin.our_publications.delete', ['id' => ':slug']) !!}";
            if (blocked) return false;
            blocked = true;
