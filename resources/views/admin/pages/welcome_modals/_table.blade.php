<table class="table align-items-center table-flush" id="datatable-basic">
    ...
    <tr>
        <th>{{ __('app.Form.Url') }}</th>
        <th>{{ __('app.Active') }}</th>
        <th class="text-right">{{ __('app.Action') }}</th>
    </tr>
    </thead>
    <tbody class="list table-sortable" data-action="{{ route('admin.welcome_modals.sort') }}">
    @forelse($items as $item)
        <tr data-id="{{ $item->id }}" class="item-row">
            <td class="item-title">{{ $item->url }}</td>
            <td>
            ...
            <td class="text-right">
                <a class="btn btn-sm btn-icon-only btn-outline-primary"
                   href="{{ route('admin.welcome_modals.edit', ['id' => $item->id]) }}"
                   title="{{ __('app.Edit') }}">
