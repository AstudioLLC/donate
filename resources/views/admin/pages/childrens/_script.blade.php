...
@push('js')
    ...
    <script>
        ...
        const showUrl = '{{ route('admin.childrens.show', ['id' => ':slug']) }}';

        const galleryTooltip = '{!! __('app.Gallery') !!}';
        const galleryUrl = '{{ route('admin.gallery.index', ['gallery' => 'childrens', 'key' => ':slug']) }}';

        const fileTooltip = '{!! __('app.Files') !!}';
        const fileUrl = '{{ route('admin.file.index', ['file' => 'childrens', 'key' => ':slug']) }}';

        const videoTooltip = '{!! __('app.Video gallery') !!}';
        const videoUrl = '{{ route('admin.videos.index', ['video' => 'childrens', 'key' => ':slug']) }}';

        const editTooltip = '{!! __('app.Edit') !!}';
        const editUrl = '{{ route('admin.childrens.edit', ['id' => ':slug']) }}';
        ...
        $('.dataTable').dataTable({
            ...
            //order: [],
            ajax: {
                url: '{{ route('admin.childrens.listPortion') }}',
                type: 'POST',
                /*data: function (e) {
        ...
        $(document).on('click', '.active-changer span', function() {
            ...
            let url = "{!! route('admin.childrens.active', ['id' => ':slug']) !!}";
        ...
        $('#itemDeleteForm').on('submit', function () {
            let url = "{!! route('admin.childrens.delete', ['id' => ':slug']) !!}";
            if (blocked) return false;
            blocked = true;
