<table class="table align-items-center table-flush" id="datatable-basic">
    ...
    <tbody class="list table-sortable" data-action="{{ route('admin.corporate_donors.sort') }}">
    @forelse($items as $item)
        <tr data-id="{{ $item->id }}" class="item-row">
            <td class="item-title">{{ $item->title }}</td>
            <td>
                <label class="custom-toggle is_our_donor-changer">
                    <input type="checkbox" value="{{ $item->is_our_donor }}" {{ $item->is_our_donor ? ' checked' : '' }}>
            ...
            <td class="text-right">
                <a class="btn btn-sm btn-icon-only btn-outline-primary"
                   href="{{ route('admin.corporate_donors.edit', ['id' => $item->id]) }}"
                   title="{{ __('app.Edit') }}">
