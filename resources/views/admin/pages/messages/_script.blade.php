...
@push('js')
    ...
    <script>
        ...
        const showUrl = '{{ route('admin.messages.show', ['id' => ':slug']) }}';

        const editTooltip = '{!! __('app.Edit') !!}';
        const editUrl = '{{ route('admin.messages.edit', ['id' => ':slug']) }}';
        ...
        $('.dataTable').dataTable({
            ...
            //order: [],
            ajax: {
                url: '{{ route('admin.messages.listPortion') }}',
                type: 'POST',
                /*data: function (e) {
        ...
        $('#itemDeleteForm').on('submit', function () {
            let url = "{!! route('admin.messages.delete', ['id' => ':slug']) !!}";
            if (blocked) return false;
            blocked = true;
