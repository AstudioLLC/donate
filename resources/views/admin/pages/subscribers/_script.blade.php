...
@push('js')
    ...
    <script>
        ...
        const showUrl = '{{ route('admin.subscribers.show', ['id' => ':slug']) }}';

        const editTooltip = '{!! __('app.Edit') !!}';
        const editUrl = '{{ route('admin.subscribers.edit', ['id' => ':slug']) }}';
        ...
        $('.dataTable').dataTable({
            ...
            //order: [],
            ajax: {
                url: '{{ route('admin.subscribers.listPortion') }}',
                type: 'POST',
                /*data: function (e) {
        ...
        $(document).on('click', '.active-changer span', function() {
            ...
            let url = "{!! route('admin.subscribers.active', ['id' => ':slug']) !!}";
        ...
        $('#itemDeleteForm').on('submit', function () {
            let url = "{!! route('admin.subscribers.delete', ['id' => ':slug']) !!}";
            if (blocked) return false;
            blocked = true;
