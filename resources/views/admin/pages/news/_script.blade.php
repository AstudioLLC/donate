...
@push('js')
    ...
    <script>
        ...
        const showUrl = '{{ route('admin.news.show', ['id' => ':slug']) }}';

        const galleryTooltip = '{!! __('app.Gallery') !!}';
        const galleryUrl = '{{ route('admin.gallery.index', ['gallery' => 'news', 'key' => ':slug']) }}';

        const editTooltip = '{!! __('app.Edit') !!}';
        const editUrl = '{{ route('admin.news.edit', ['id' => ':slug']) }}';
        ...
        $('.dataTable').dataTable({
            ...
            //order: [],
            ajax: {
                url: '{{ route('admin.news.listPortion') }}',
                type: 'POST',
                /*data: function (e) {
        ...
        $(document).on('click', '.active-changer span', function() {
            ...
            let url = "{!! route('admin.news.active', ['id' => ':slug']) !!}";
        ...
        $('#itemDeleteForm').on('submit', function () {
            let url = "{!! route('admin.news.delete', ['id' => ':slug']) !!}";
            if (blocked) return false;
            blocked = true;
